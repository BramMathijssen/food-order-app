...
const Meal = (props) => {
    ...
    return (
        <div className="meal" ref={scrollRef}>
            ...
            <form className="controls" onSubmit={submitHandler}>
                ...
                <button className="controls__add btn btn--circle" type="submit" onClick={onClickHandler}>
                    {added ? <IonIcon className="button--icon" icon={checkmarkOutline} /> : <IonIcon className="button--icon" icon={addOutline} />}
                </button>
            </form>
        </div>
    );
